...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model=hp.D_MODEL, num_heads=hp.HEADS, dropout=0.1):
        ...
        self.linear_Qs = nn.ModuleList([nn.Linear(d_model, self.d).to(device)
                                        for _ in range(num_heads)])
        self.linear_Ks = nn.ModuleList([nn.Linear(d_model, self.d).to(device)
                                        for _ in range(num_heads)])
        self.linear_Vs = nn.ModuleList([nn.Linear(d_model, self.d).to(device)
                                        for _ in range(num_heads)])
...
if __name__ == "__main__":
    ...
    if not os.path.exists(MODEL_PATH):
        ...
        print("MHA Architecture", transformer.encoder.encoders[0].mha)
